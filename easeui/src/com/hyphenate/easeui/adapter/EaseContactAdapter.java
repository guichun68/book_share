...
//                final int count = mOriginalList.size();//austin annotation(解决:当用户回退输入框字符串时，不起作用)
                final int count = copyUserList.size();//austin+
                final ArrayList<EaseUser> newValues = new ArrayList<EaseUser>();
                for(int i=0;i<count;i++){
//                    final EaseUser user = mOriginalList.get(i);//austin annotation(解决:当用户回退输入框字符串时，不起作用)
                    final EaseUser user = copyUserList.get(i);
//                    String username = user.getUsername();//austin annotation(应该过滤昵称，而非环信系统的userId)
