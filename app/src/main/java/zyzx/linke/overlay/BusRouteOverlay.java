...
public class BusRouteOverlay extends RouteOverlay {
	...
	public BusRouteOverlay(Context context, AMap amap, BusPath path,
		...
		super(context);
		this.busPath = path;
		startPoint = SchemeUtil.convertToLatLng(start);
		endPoint = SchemeUtil.convertToLatLng(end);
		mAMap = amap;
	}
		...
		try {
			...
			for (int i = 0; i < busSteps.size(); i++) {
				...
				if (busStep.getBusLine() != null) {
					...
					if (i == busSteps.size() - 1) {
						addWalkPolyline(SchemeUtil.convertToLatLng(getLastBuslinePoint(busStep)), endPoint);
					}
				}
				if (busStep.getRailway() != null) {
					...
					if (i == busSteps.size() - 1) {
						addWalkPolyline(SchemeUtil.convertToLatLng(busStep.getRailway().getArrivalstop().getLocation()), endPoint);
					}
				}
	...
	private void addRailwayStep(RouteRailwayItem railway) {
		...
		for (int i = 0; i < railwayStationItems.size(); i++) {
			railwaylistpoint.add(SchemeUtil.convertToLatLng(railwayStationItems.get(i).getLocation()));
		}
	...
	private void addTaxiStep(TaxiItem taxi){
		addPolyLine(new PolylineOptions().width(getRouteWidth())
				.color(getBusColor())
				.add(SchemeUtil.convertToLatLng(taxi.getOrigin()))
				.add(SchemeUtil.convertToLatLng(taxi.getDestination())));
	}
	...
	private void addWalkSteps(BusStep busStep) {
		...
		for (int j = 0; j < walkSteps.size(); j++) {
			...
			if (j == 0) {
				LatLng latLng = SchemeUtil.convertToLatLng(walkStep
						.getPolyline().get(0));
			...
			List<LatLng> listWalkPolyline = SchemeUtil
			...
			if (j < walkSteps.size() - 1) {
				LatLng lastLatLng = listWalkPolyline.get(listWalkPolyline
						.size() - 1);
				LatLng firstlatLatLng = SchemeUtil
	...
	private void addBusLineSteps(List<LatLonPoint> listPoints) {
		...
		addPolyLine(new PolylineOptions().width(getRouteWidth())
				.color(getBusColor())
				.addAll(SchemeUtil.convertArrList(listPoints)));
	}
	...
	private void addBusStationMarkers(RouteBusLineItem routeBusLineItem) {
		...
		LatLng position = SchemeUtil.convertToLatLng(startBusStation
				.getLatLonPoint());
	...
	private void addTaxiMarkers(TaxiItem taxiItem) {
		...
		LatLng position = SchemeUtil.convertToLatLng(taxiItem
				.getOrigin());
	...
	private void addRailwayMarkers(RouteRailwayItem railway) {
		LatLng Departureposition = SchemeUtil.convertToLatLng(railway
		...
		LatLng Arrivalposition = SchemeUtil.convertToLatLng(railway
	...
	private void checkBusToNextBusNoWalk(BusStep busStep, BusStep busStep1) {
		LatLng endbusLatLng = SchemeUtil
				.convertToLatLng(getLastBuslinePoint(busStep));
		LatLng startbusLatLng = SchemeUtil
	...
	private void checkBusEndToNextBusStart(BusStep busStep, BusStep busStep1) {
		LatLonPoint busLastPoint = getLastBuslinePoint(busStep);
		LatLng endbusLatLng = SchemeUtil.convertToLatLng(busLastPoint);
		LatLonPoint busFirstPoint = getFirstBuslinePoint(busStep1);
		LatLng startbusLatLng = SchemeUtil.convertToLatLng(busFirstPoint);
	...
	private void addWalkPolyLineByLatLonPoints(LatLonPoint pointFrom,
			LatLonPoint pointTo) {
		LatLng latLngFrom = SchemeUtil.convertToLatLng(pointFrom);
		LatLng latLngTo = SchemeUtil.convertToLatLng(pointTo);
