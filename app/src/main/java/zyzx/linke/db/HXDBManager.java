...
public class HXDBManager {
    static private HXDBManager dbMgr = new HXDBManager();
    private HXDbOpenHelper dbHelper;

    private HXDBManager(){
        dbHelper = HXDbOpenHelper.getInstance(BaseApplication.getApplication());
    }
    ...
    synchronized public void saveContactList(List<EaseUser> contactList) {
        ...
        if (db.isOpen()) {
            db.delete(HXUserDao.TABLE_NAME, null, null);
    ...
    synchronized public Map<String, EaseUser> getContactList() {
        ...
        if (db.isOpen()) {
            Cursor cursor = db.rawQuery("select * from " + HXUserDao.TABLE_NAME /* + " desc" */, null);
            while (cursor.moveToNext()) {
                String username = cursor.getString(cursor.getColumnIndex(HXUserDao.COLUMN_NAME_ID));
                String nick = cursor.getString(cursor.getColumnIndex(HXUserDao.COLUMN_NAME_NICK));
                String avatar = cursor.getString(cursor.getColumnIndex(HXUserDao.COLUMN_NAME_AVATAR));
                EaseUser user = new EaseUser(username);
                user.setNick(nick);
                user.setAvatar(avatar);
                if (username.equals(MyEaseConstant.NEW_FRIENDS_USERNAME) || username.equals(MyEaseConstant.GROUP_USERNAME)
                        || username.equals(MyEaseConstant.CHAT_ROOM)|| username.equals(MyEaseConstant.CHAT_ROBOT)) {
    ...
    synchronized public void deleteContact(String username){
        SQLiteDatabase db = dbHelper.getWritableDatabase();
        if(db.isOpen()){
            db.delete(HXUserDao.TABLE_NAME, HXUserDao.COLUMN_NAME_ID + " = ?", new String[]{username});
        }
    }
    ...
    public void setDisabledGroups(List<String> groups){
        setList(HXUserDao.COLUMN_NAME_DISABLED_GROUPS, groups);
    }

    public List<String>  getDisabledGroups(){
        return getList(HXUserDao.COLUMN_NAME_DISABLED_GROUPS);
    }

    public void setDisabledIds(List<String> ids){
        setList(HXUserDao.COLUMN_NAME_DISABLED_IDS, ids);
    }

    public List<String> getDisabledIds(){
        return getList(HXUserDao.COLUMN_NAME_DISABLED_IDS);
    }
    ...
    synchronized private void setList(String column, List<String> strList){
        ...
        if (db.isOpen()) {
            ...
            db.update(HXUserDao.PREF_TABLE_NAME, values, null,null);
        }
    }

    synchronized private List<String> getList(String column){
        SQLiteDatabase db = dbHelper.getReadableDatabase();
        Cursor cursor = db.rawQuery("select " + column + " from " + HXUserDao.PREF_TABLE_NAME,null);
    ...
    synchronized public void saveRobotList(List<RobotUser> robotList) {
        SQLiteDatabase db = dbHelper.getWritableDatabase();
        if (db.isOpen()) {
            db.delete(HXUserDao.ROBOT_TABLE_NAME, null, null);
            for (RobotUser item : robotList) {
                ContentValues values = new ContentValues();
                values.put(HXUserDao.ROBOT_COLUMN_NAME_ID, item.getUsername());
                if (item.getNick() != null)
                    values.put(HXUserDao.ROBOT_COLUMN_NAME_NICK, item.getNick());
                if (item.getAvatar() != null)
                    values.put(HXUserDao.ROBOT_COLUMN_NAME_AVATAR, item.getAvatar());
                db.replace(HXUserDao.ROBOT_TABLE_NAME, null, values);
            }
        }
    }
    ...
    synchronized public Map<String, RobotUser> getRobotList() {
        ...
        if (db.isOpen()) {
            Cursor cursor = db.rawQuery("select * from " + HXUserDao.ROBOT_TABLE_NAME, null);
            if(cursor.getCount()>0){
                users = new Hashtable<String, RobotUser>();
            }
            while (cursor.moveToNext()) {
                String username = cursor.getString(cursor.getColumnIndex(HXUserDao.ROBOT_COLUMN_NAME_ID));
                String nick = cursor.getString(cursor.getColumnIndex(HXUserDao.ROBOT_COLUMN_NAME_NICK));
                String avatar = cursor.getString(cursor.getColumnIndex(HXUserDao.ROBOT_COLUMN_NAME_AVATAR));
