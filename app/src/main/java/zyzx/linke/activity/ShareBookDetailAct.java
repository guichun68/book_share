...
public class ShareBookDetailAct extends BaseActivity {
    ...
    private boolean isInRelsBlackList;//当前登录用户是否被mFriend加入了黑名单
    ...
    public void onClick(View view) {
        ...
        switch (view.getId()){
            ...
            case R.id.btn_sharer://好友名字
                ...
                ex.putBoolean(BundleFlag.IS_IN_FRIENDS_BLACLIST,isInRelsBlackList);
    ...
    private void showBegDialog() {
        begDialog =
        CustomProgressDialog.getPromptDialog2Btn(this,"确定向对方发送"+btnBegBorrow.getText().toString()+"消息么？","确定","取消",new DialogListener(),null);
        begDialog.show();
    }
...
//                dismissProgress();
    ...
    private void uploadBegRecord(){
        getUserPresenter().sendBegBookMsg(mBookVo.getShareType(),GlobalParams.getLastLoginUser().getUserid(),mFriend,mBook,new CallBack(){
...
//                        newBookStatusId = (String)((Map)drj.getData().getItems().get(1)).get("dis");
        ...
        getUserPresenter().getUserInfoByUid2(mBookVo.getUid(), new CallBack() {
            ...
            public void onSuccess(Object obj, int... code) {
                ...
                isInRelsBlackList = ((JSONObject)ja.get(1)).getBoolean("isInRelsBlackList");
